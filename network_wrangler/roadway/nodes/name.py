...
def add_roadway_link_names_to_nodes(roadway_network: RoadwayNetwork) -> None:  # noqa: PLR0912, PLR0915
    ...
    string_names = roadway_network.links_df[
        roadway_network.links_df["name"].apply(lambda x: isinstance(x, str))
    ]
    ...
    roadway_network.nodes_df["incoming_link_names"] = [
        [] for _ in range(len(roadway_network.nodes_df))
    ]
    roadway_network.nodes_df["outgoing_link_names"] = [
        [] for _ in range(len(roadway_network.nodes_df))
    ]
    ...
    for idx, row in roadway_network.nodes_df.iterrows():
        ...
        if node_id in incoming_names:
            roadway_network.nodes_df.at[idx, "incoming_link_names"] = sorted(
                incoming_names[node_id]
            )
        if node_id in outgoing_names:
            roadway_network.nodes_df.at[idx, "outgoing_link_names"] = sorted(
                outgoing_names[node_id]
            )
    ...
    if 1011109 in roadway_network.nodes_df["model_node_id"].values:  # noqa: PLR2004
        node_row = roadway_network.nodes_df[
            roadway_network.nodes_df["model_node_id"] == 1011109  # noqa: PLR2004
        ].iloc[0]
